...
Variable names are: bear_id, capture_date, popn, sex, age_class, mating, denning, lat, long, urea_nitrogen, and creatinine. 
...
NA's are represented by "unknown."
...
```{r}
sample_count <- polar_bears_clean %>% 
  group_by(popn) %>% 
  count(sex)
...
sample_count %>% 
  ggplot(
    ...
    facet_wrap(~popn)+
    theme_classic()+
    labs(title = "Polar Bear Population Sample Counts",
         x = "Sex by Population",
         y = "Count",
         fill = "Sex")
...
polar_bears %>%
  group_by(popn) %>% 
  mutate(year = (year(ymd(capture_date)))) %>% 
...
For urea_nitrogen the min is 1.00, the median is 11.00, the mean is 15.13, and the max is 69.00.

For creatinine the min is 0.250, the median is 1.050, the mean is 1.079, and the max is 2.350.
...
```{r}
polar_bears_ratio <- polar_bears_clean %>% 
  mutate(
    un_creat_ratio = (urea_nitrogen * 0.466) / creatinine
  )
```
...
```{r}
polar_bears_fasting <- polar_bears_ratio %>% 
  mutate(
    fasting = case_when(un_creat_ratio <= 10 ~ "yes",
...
polar_bears_fasting %>%
  ...
  theme_classic()+
  labs(
    title = "Count of Fasting Bears per Population per Year",
    x = "Year By Population",
    y = "Count",
    color = "Population"
  )
...
ggmap(map1)+
  geom_point(data=polar_bears_fasting,
             aes(long,
                 lat,
                 color = popn),
             size = 0.75,
  ...
  labs(
    title = "Map of Sampled Polar Bear Locations",
    color = "Population",
    x = "Longitude",
    y = "Latitude"
  )
...
ui <- fluidPage(
  ...
  selectInput("x", "Select X Variable", choices = c("sex",
                            "mating",
                            "age_class",
                            "denning",
                            "fasting"),
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    polar_bears_fasting %>% 
      ...
      labs(
        title = "Range of Urea Nitrogen / Creatinine Ratio by Selected Variable By Population",
        x = "Selected Variable by Population",
        y = "Urea Nitrogen / Creatinine Ratio",
        fill = "Population"
      )
  })
}
